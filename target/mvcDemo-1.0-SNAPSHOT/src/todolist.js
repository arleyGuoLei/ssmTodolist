    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if(typeof data === 'object' && data !== null){
          if(data.status === true){
            document.getElementById("username").innerText = "user:" + data.data;
            local_notes_key = data.data
            init()
          }else{
...
function init() {     //页面初始化开始
  if (!localStorage[local_notes_key]) local_notes = [A_status]   //如果第一次加载页面则不必要读取localStorage
  else {
    init_status = true   //进入初始化状态
    local_notes = JSON.parse(localStorage[local_notes_key])
...
function update_localStorage(num, value) {
  ...
  localStorage[local_notes_key] = JSON.stringify(local_notes)
}
